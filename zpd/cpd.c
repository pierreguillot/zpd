...
z_instance* z_pd_instance_new(size_t size)
{
    ...
    if(internal)
    {
        ...
        {
            pdinstance_free(internal->z_intance);
            free(internal);
            return NULL;
        }
    }
...
z_atomtype z_pd_list_get_type(z_list const* list, size_t index)
{
...
void* z_pd_list_get_atoms(z_list* list, size_t index)
{
    return (void *)list->l_vec+index;
}

z_atomtype cpd_atom_get_type(z_atom const* atom)
{
    return atom->a_type;
}

z_float cpd_atom_get_float(z_atom const* atom)
{
    return atom->a_w.w_float;
}

z_symbol* cpd_atom_get_symbol(z_atom const* atom)
{
    return atom->a_w.w_symbol;
}

void cpd_atom_set_float(z_atom* atom, z_float value)
{
    atom->a_type = Z_FLOAT;
    atom->a_w.w_float = value;
}

void cpd_atom_set_symbol(z_atom* atom, z_symbol* symbol)
{
    atom->a_type = Z_FLOAT;
